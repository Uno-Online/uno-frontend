        ...
        <form className={styles.form} onSubmit={handleSubmit(login)}>
            ...
            <div className={styles['content-form']}>
                <header className={styles['header-form']}>
                    <button type="button" className={styles['button-header-secondary']} onClick={()=>{
                        reset()
                        setLoginOrGuest('login')
                        }}>Entrar</button>
                    <button type="button" className={styles['button-header-primary']}>Convidado</button>
                </header>
                    {serverError??
                ...
                <div className={styles['input-group']}>
                        <label htmlFor="name">
                            <span>Nome</span>
                        <input type="text" className={errors.name?.message?`${styles['input-error']}`:''}  {...register('name')} placeholder='Escolha seu nick' />
                        {errors.name?.message?<small className={styles['form-error']}>{errors.name?.message}</small>:''}
                        </label>                    
                        <label htmlFor="number">
                            <span>Sala</span>
                        <input type="number" className={errors.room?.message?`${styles['input-error']}`:''}  {...register('room')} placeholder='Digite o código da sala'/>
                        {errors.room?.message?<small className={styles['form-error']}>{errors.room?.message}</small>:''}
                        </label>
                                                                              
               </div>
               <button aria-label='Entrar' type='submit'disabled={isSubmitting} className={styles['button-form']}> Entrar </button>
